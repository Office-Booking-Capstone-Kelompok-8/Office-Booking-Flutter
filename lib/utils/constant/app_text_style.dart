import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'app_colors.dart';

TextStyle titleAppBar = TextStyle(
  color: AppColors.neutral10,
  fontSize: 18.sp,
  fontWeight: FontWeight.w700,
);
TextStyle formTop = TextStyle(
  color: AppColors.neutral8,
  fontSize: 12.sp,
  fontWeight: FontWeight.w400,
);
TextStyle buttonStyle = TextStyle(
  color: AppColors.neutral2,
  fontSize: 16.sp,
  fontWeight: FontWeight.w700,
);
TextStyle ratingStyle = TextStyle(
  color: AppColors.neutral2,
  fontSize: 12.sp,
  fontWeight: FontWeight.w400,
);
TextStyle detailFormStyle = TextStyle(
  color: AppColors.neutral9,
  fontSize: 14.sp,
  fontWeight: FontWeight.w700,
);
TextStyle detailFormGrey = TextStyle(
  color: AppColors.neutral8,
  fontSize: 12.sp,
  fontWeight: FontWeight.w400,
);
TextStyle onboardSkip = TextStyle(
  color: AppColors.primary4,
  fontSize: 14.sp,
  fontWeight: FontWeight.w700,
);
TextStyle onboardTitle = TextStyle(
  color: AppColors.neutral10,
  fontSize: 16.sp,
  fontWeight: FontWeight.w700,
);
TextStyle titleReservation = TextStyle(
  color: AppColors.neutral10,
  fontSize: 14.sp,
  fontWeight: FontWeight.w700,
);
TextStyle onboardSubtitle = TextStyle(
  color: AppColors.neutral11,
  fontSize: 14.sp,
  fontWeight: FontWeight.w400,
);
TextStyle personReservation = TextStyle(
  color: AppColors.neutral10,
  fontSize: 14.sp,
  fontWeight: FontWeight.w400,
);

TextStyle dateReservation = TextStyle(
  color: AppColors.primary7,
  fontSize: 12.sp,
  fontWeight: FontWeight.w400,
);
TextStyle textEmptyReservation = TextStyle(
  color: AppColors.neutral9,
  fontSize: 14.sp,
  fontWeight: FontWeight.w700,
);
TextStyle idTransaction = TextStyle(
  color: AppColors.neutral10,
  fontSize: 12.sp,
  fontWeight: FontWeight.w400,
);
TextStyle dateOrder = TextStyle(
  color: AppColors.neutral8,
  fontSize: 14.sp,
  fontWeight: FontWeight.w700,
);
TextStyle priceOrder = TextStyle(
  color: AppColors.error5,
  fontSize: 16.sp,
  fontWeight: FontWeight.w700,
);

TextStyle buildingName = TextStyle(
  color: AppColors.neutral9,
  fontSize: 16.sp,
  fontWeight: FontWeight.w700,
);
TextStyle detailBuilidingStyle = TextStyle(
  color: AppColors.neutral9,
  fontSize: 12.sp,
  fontWeight: FontWeight.w400,
);
TextStyle priceBold = TextStyle(
  color: AppColors.neutral9,
  fontSize: 12.sp,
  fontWeight: FontWeight.w700,
);
