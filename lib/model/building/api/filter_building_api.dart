  ...
  Future<List<BuildingModel>?> getAllBuildings(
    ...
    try {
      final response = await _dio.get(
          '${Api.baseUrl}${Api.building}?limit=10&page=1&city=${(data['location'] != null) ? data['location'].id : ''}&startDate=${data['date'] ?? ''}&duration=${data['duration'] ?? ''}&capacityMin=${data['capacityMin'] ?? ''}&capacityMax=${data['capacityMax'] ?? ''}');
