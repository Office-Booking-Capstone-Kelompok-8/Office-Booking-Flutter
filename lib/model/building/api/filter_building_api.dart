  ...
  Future<List<BuildingModel>?> getAllBuildings(
    ...
    try {
      final response = await _dio.get(
          '${Api.baseUrl}${Api.building}?limit=10&page=1&cityId=${(data['location'] != null) ? data['location'].id : ''}${(data['date'] != null) ? '&startDate=${data['date']}' : ''}&duration=${data['duration'] ?? ''}&capacityMin=${data['capacityMin'] ?? ''}&capacityMax=${data['capacityMax'] ?? ''}&order=${data['order'] ?? ''}&sortBy=${data['sortBy'] ?? ''}');
